...
const PickItem = ({ goods }) => {
  ...
  const[alert,setAlert] = useState(false)
    ...
    <Box bg={"white"} display="flex" flexDirection={{base:"column",md:"row"}} flexWrap={"wrap"} justifyContent={"space-between"}   p={3} height={"max-content"}>
        ...
        {alert ? <AlertDialogExample time={time} alert= {alert} setAlert={setAlert} onOpen={onOpen} onClose={onClose} /> : null}
...
function AlertDialogExample({time,alert,setAlert, onOpen, onClose }) {
