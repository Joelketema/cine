...
const Discussion = ({title}) => {
    ...
    return (
        ...
        <Box display={"flex"} flexDirection={"column"} bg={"orangered"} p={3} height={"max-content"} borderRadius={10}>    
        <Grid templateColumns='repeat(1, 1fr)' templateRows='repeat(4, 1fr)' gap={5}>
            ...
            <Box display={"flex"} flexDirection={"column"} justifyContent={"center"} alignItems={"center"} gap={15}>
                <Input variant='outline' placeholder='Be A Part of the Conversation!' mt={5} />
                <Button bg={"black"} color="white" variant="solid">Post</Button> 
            </Box>    
